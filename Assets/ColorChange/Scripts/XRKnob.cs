...
namespace UnityEngine.XR.Content.Interaction
{
    namespace ColorChanger
    {
        ...
        public class XRKnob : XRBaseInteractable
        {
            ...
            private const int requiredRotations = 2;
            ...
            void SetValue(float value)
            {
                ...
                // Check if the knob has rotated requiredRotations times or more
                if (Mathf.Abs(m_Value - 1.0f) < 0.001f)
                {
                    rotationCount++;
                    if (rotationCount >= requiredRotations)
                    {
                        ActivateObject();
                        rotationCount = 0; // Reset rotation count after activation
                        /*
                        if (m_Value >= 15.0f ) // 8번 돌렸을 때 액체 나오도록 설정
                        {
                            //ColorChanger.TriggerArea 클래스의 pourCheck 변수에 접근
                            TriggerArea triggerArea = new TriggerArea();
                            triggerArea.pourCheck = true;
                        }
                        */
                    }
                }
